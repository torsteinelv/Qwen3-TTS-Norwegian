...
1. ✅ Utvider språkembedding-tabellen med "Norwegian" (riktig sti: talker.model.language_embedding)
...
try:
    ...
    print("Sjekk at mappen '/workspace/Qwen3-TTS' eksisterer og inneholder inference/qwen3_tts_model.py")
    sys.exit(1)


# ==========================================
# 1. UTVID SPRÅKEMBEDDING MED NORSK (RIKTIG STI!)
# ==========================================
def extend_language_embedding(model):
    """
    Legger til "Norwegian" i språkembedding-tabellen.
    VIKTIG: language_embedding ligger på model.model.talker.model, IKKE model.model.talker
    """
    # ✅ RIKTIG STI: talker.model.language_embedding
    current_weights = model.model.talker.model.language_embedding.weight.data.clone()
    ...
    # ✅ RIKTIG OPPDATERING: talker.model.language_embedding
    model.model.talker.model.language_embedding.weight = torch.nn.Parameter(
        new_weight, 
...
def train():
    ...
    # ✅ BRUK dtype I STEDET FOR deprecated torch_dtype
    model = Qwen3TTSModel.from_pretrained(
        args.init_model_path,
        dtype=torch.bfloat16,  # IKKE torch_dtype!
        device_map={"": accelerator.device},
        attn_implementation="flash_attention_2" if torch.cuda.is_available() else None
    )
    
    # 1. Utvid språkembedding (med RIKTIG sti)
    model = extend_language_embedding(model)
    
    # 2. Verifiser at "Norwegian" ble lagt til
    if "Norwegian" not in model.model.config.talker_config.languages:
        print("❌ FEIL: 'Norwegian' ble IKKE lagt til i språklista!")
        print(f"   Språkliste: {model.model.config.talker_config.languages}")
        sys.exit(1)
    
    norwegian_lang_id = model.model.config.talker_config.languages.index("Norwegian")
    print(f"✅ Verifisert: 'Norwegian' har språk-ID {norwegian_lang_id}")
    ...
    # 5. Aktiver trening på spesifikke lag (RIKTIG STI!)
    model.model.talker.model.language_embedding.weight.requires_grad = True
    ...
    if accelerator.is_main_process:
        ...
        print(f"   - Language embedding: {model.model.talker.model.language_embedding.weight.requires_grad}")
    ...
    for epoch in range(args.num_epochs):
        ...
        for step, batch in enumerate(dataloader):
            with accelerator.accumulate(model):
                ...
                # ✅ RIKTIG STI: talker.model.language_embedding
                lang_embeds = model.model.talker.model.language_embedding(language_ids)
        ...
        if accelerator.is_main_process:
            ...
            if (epoch + 1) % args.save_every == 0:
                ...
                torch.save(unwrapped_model.model.talker.model.language_embedding.state_dict(), os.path.join(save_path, "language_embedding.bin"))
                ...
                # ✅ SIKKER README-SKRIVING (ingen syntax errors)
                readme_lines = [
                    "---",
                    ...
                    '    language="Norwegian",  # VIKTIG: eksakt dette navnet!',
                    '    ref_audio="ref.wav"',
                    ")",
                    "```"
                ]
